  ...
  return (
    <div className={GTL.wrap} style={{ fontFamily, fontSize }}>
      {tasks.map((t) => {
        ...
        return (
          <div
            className={cn(GTL.row, selectedTaskId === t.id && "ring-1 ring-inset ring-[#B96A2D]/40")}
              ...
              <div className={GTL.nameWrap}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {t.type === "task" && !isLeaveTaskId(String(t.id)) && (
                    <div className="text-[11px] font-medium tabular-nums text-zinc-500">
...
export default function PlannerGantt({
    ...
    () =>
      function PlannerGanttTooltip({
        ...
        if (task.type === "project") {
          return (
            <div className="max-w-xs rounded-md border border-zinc-200 bg-white p-3 text-sm shadow" style={s}>
        ...
        if (isLeaveTaskId(tid)) {
          ...
          const dur = leave ? calendarSpanInclusiveDays(leave.start_date, leave.end_date) : 0;
          return (
            <div className="max-w-xs rounded-md border border-zinc-200 bg-white p-3 text-sm shadow" style={s}>
              <p className="font-semibold">
                {leave?.person_name?.trim() ? `Leave — ${leave.person_name.trim()}` : "People leave"}
              </p>
              <p className="mt-1 text-xs text-zinc-500">Crew: {crewName}</p>
        ...
        return (
          <div className="max-w-xs rounded-md border border-zinc-200 bg-white p-3 text-sm shadow" style={s}>
            ...
            <p className="mt-1 text-xs text-zinc-500">Crew: {crewName}</p>
            <p className="text-xs text-zinc-500">Status: {act.status}</p>
            <p className="mt-1 text-xs">
              {formatTooltipDay(task.start)} – {formatTooltipDay(task.end)}
            </p>
            <p className="text-xs text-zinc-500">
  ...
  if (tasks.length === 0) {
    return (
      <div className="flex h-64 items-center justify-center rounded-lg border border-zinc-200 bg-white text-sm text-zinc-500">
  ...
  return (
    <div className="planner-gantt flex h-[min(72vh,calc(100dvh-13rem))] min-h-[280px] w-full max-w-full flex-col overflow-hidden rounded-xl border border-border bg-card p-4 shadow-sm">
      <div className="mb-3 flex shrink-0 flex-wrap items-center gap-x-4 gap-y-2 border-b border-border pb-3 text-xs text-zinc-600">
        <span className="font-medium text-zinc-500">Legend</span>
        <span className="inline-flex items-center gap-1.5">
          <span className="h-2.5 w-4 shrink-0 rounded-sm border border-border/60" style={{ backgroundColor: ACTIVITY_STATUS_COLORS.planned }} aria-hidden />
          Planned
        </span>
        <span className="inline-flex items-center gap-1.5">
          <span className="h-2.5 w-4 shrink-0 rounded-sm border border-border/60" style={{ backgroundColor: ACTIVITY_STATUS_COLORS.in_progress }} aria-hidden />
          In progress
        </span>
        <span className="inline-flex items-center gap-1.5">
          <span className="h-2.5 w-4 shrink-0 rounded-sm border border-border/60" style={{ backgroundColor: ACTIVITY_STATUS_COLORS.done }} aria-hidden />
          Done
        </span>
        <span className="inline-flex items-center gap-1.5">
          <span className="h-2.5 w-4 shrink-0 rounded-sm border border-border/60" style={{ backgroundColor: PEOPLE_LEAVE_BAR_COLOR }} aria-hidden />
          People leave
        </span>
      </div>
      <div ref={chartMountRef} className="min-h-0 min-w-0 flex-1 overflow-hidden">
        <Gantt
          ...
          barCornerRadius={4}
          todayColor="rgba(185, 106, 45, 0.12)"
